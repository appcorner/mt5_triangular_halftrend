...
def get_index_signal(symbol, idx, config=indicator_config):
    df = all_candles[symbol]
    is_long = df['trend'][idx] == 'long'
    is_short = df['trend'][idx] == 'long'
    return is_long, is_short
...
async def chart(symbol, timeframe, config=indicator_config, showMACDRSI=False, fiboData=None):
    ...
    try:
        ...
        if showFibo:
            ...
            fibo_position = f':: {fiboData["position"]} '
        ...
        title = f'{symbol} :: HalfTrend {fibo_position}:: ({timeframe} @ {data.index[-1]})'
        print(title)
